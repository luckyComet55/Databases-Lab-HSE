...
namespace db {
    ...
    namespace fs {
        ...
        assignment_manager::assignment_manager(const std::filesystem::path& root_dir, bool is_new) : root_dir(root_dir) {
            if (is_new) {
                return;
            }
        ...
        db::meta::TransactionStatus assignment_manager::update_record(
            ...
            for (auto i : check_records.second) {
                ...
                std::vector<std::string> view{contents.second[1]};
                bool same = true;
                ...
                auto check_duplicate = find_containers_where_clause(view, {1});
        ...
        db::meta::TransactionStatus assignment_manager::delete_record(
            const std::vector<std::string>& vals_where,
            const std::vector<int>& fields_where
        ) {
            auto check_records = find_containers_where_clause(vals_where, fields_where);
        ...
        std::pair<db::meta::TransactionStatus, std::vector<std::string>> assignment_manager::find_record(
            const std::vector<std::string>& vals_where,
            const std::vector<int>& fields_where
        ) {
        ...
        db::meta::TransactionStatus assignment_manager::insert_record(const std::vector<std::string>& entity) {
            auto check_exists = find_record(entity, {1});
        ...
        std::pair<bool, std::vector<int>> assignment_manager::find_containers_where_clause(
            const std::vector<std::string>& vals_where,
            const std::vector<int>& fields_where
        ) {
            std::string id = "";
